<?php $__env->startSection('title', 'Release Tracker | FleetMan'); ?>
<?php $__env->startSection('mobile-title', 'Release Tracker'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.fleetman.topbar','data' => ['items' => [['label' => 'System'], ['label' => 'Release Tracker']]]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['items' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute([['label' => 'System'], ['label' => 'Release Tracker']])]); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.fleetman.title-card','data' => ['title' => 'Release Tracker','subtitle' => 'Track application versions, release dates, changes, deployment environments, and known issues.']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Release Tracker','subtitle' => 'Track application versions, release dates, changes, deployment environments, and known issues.']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
        ...
        <form method="POST" action="<?php echo e(route('fleet.release-tracker.store')); ?>">
            ...
            <div class="release-form-grid">
                <div class="field">
                    ...
                    <input id="releaseVersion" name="version" value="<?php echo e(old('_release_id') ? '' : old('version')); ?>" placeholder="Example: v1.4.0" maxlength="60" required>
                </div>
                <div class="field release-title-field">
                    <label for="releaseTitle">Release Title <span class="req">*</span></label>
                    <input id="releaseTitle" name="title" value="<?php echo e(old('_release_id') ? '' : old('title')); ?>" placeholder="Example: Driver image and permission update" maxlength="255" required>
                </div>
                <div class="field fleet-form-temporal-field">
                    <label for="releaseDate">Release Date <span class="req">*</span></label>
                    <input id="releaseDate" type="date" name="release_date" value="<?php echo e(old('_release_id') ? now('Asia/Dhaka')->format('Y-m-d') : old('release_date', now('Asia/Dhaka')->format('Y-m-d'))); ?>" required>
                </div>
                <div class="field">
                    ...
                    <select id="releaseEnvironment" name="environment" required>
                        <?php $__currentLoopData = $environmentOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($value); ?>" <?php if(!old('_release_id') && old('environment', 'production') === $value): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                ...
                <div class="field">
                    ...
                    <select id="releaseStatus" name="status" required>
                        <?php $__currentLoopData = $statusOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($value); ?>" <?php if(!old('_release_id') && old('status', 'draft') === $value): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="field release-summary-field">
                    <label for="releaseSummary">Summary</label>
                    <textarea id="releaseSummary" name="summary" maxlength="2000" placeholder="Briefly describe this release."><?php echo e(old('_release_id') ? '' : old('summary')); ?></textarea>
                </div>
                <div class="field release-notes-field">
                    <label for="releaseChanges">Changes / Release Notes</label>
                    <textarea id="releaseChanges" name="changes" maxlength="20000" placeholder="List completed features, fixes, and technical changes."><?php echo e(old('_release_id') ? '' : old('changes')); ?></textarea>
                </div>
                <div class="field release-notes-field">
                    <label for="releaseKnownIssues">Known Issues</label>
                    <textarea id="releaseKnownIssues" name="known_issues" maxlength="20000" placeholder="List unresolved issues or write None."><?php echo e(old('_release_id') ? '' : old('known_issues')); ?></textarea>
                </div>
            </div>
    ...
    <section class="card release-list-card">
        ...
        <form method="GET" action="<?php echo e(route('fleet.release-tracker')); ?>" class="release-filter-grid">
            <div class="field release-search-field">
                ...
                <input id="releaseSearch" name="q" value="<?php echo e($filters['search']); ?>" placeholder="Version, title, notes, or issue">
            </div>
            <div class="field">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $releases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $release): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            ...
                            <td><?php echo e($release->updatedBy?->name ?? $release->createdBy?->name ?? 'Unknown'); ?></td>
                            <td>
                                <div class="release-row-actions">
                                    <button type="button" class="mini-btn" data-release-view="<?php echo e($release->id); ?>">View</button>
                                    <button type="button" class="mini-btn" data-release-edit="<?php echo e($release->id); ?>">Edit</button>
                                    <form method="POST" action="<?php echo e(route('fleet.release-tracker.destroy', $release)); ?>" onsubmit="return confirm('Delete this release entry? This action cannot be undone.');">
                                        <?php echo csrf_field(); ?>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $releases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $release): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <article class="release-mobile-card">
                    ...
                    <div class="release-mobile-meta">
                        ...
                        <span><small>Updated By</small><b><?php echo e($release->updatedBy?->name ?? $release->createdBy?->name ?? 'Unknown'); ?></b></span>
                        <span><small>Created</small><b><?php echo e(optional($release->created_at)->timezone('Asia/Dhaka')->format('d M Y, h:i A')); ?></b></span>
                    </div>
...
<div id="releaseEditModal" class="release-modal hidden" aria-hidden="true">
    <section class="release-modal-panel" role="dialog" aria-modal="true" aria-labelledby="releaseEditTitle">
        ...
        <form id="releaseEditForm" method="POST" action="">
            ...
            <input type="hidden" name="_release_id" id="editReleaseId" value="<?php echo e(old('_release_id')); ?>">
            <div class="release-modal-body">
                <div class="release-form-grid">
                    <div class="field">
...
(() => {
    ...
    const updateUrlTemplate = <?php echo json_encode(route('fleet.release-tracker.update', ['release' => '__ID__']), 512) ?>;
    const oldEdit = <?php echo json_encode($oldEditPayload, 15, 512) ?>;
...
<?php echo $__env->make('layouts.fleetman', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/laravel/FleetManagement/resources/views/fleetman/system/release-tracker.blade.php ENDPATH**/ ?>